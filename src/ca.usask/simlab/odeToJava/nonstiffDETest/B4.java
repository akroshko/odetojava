...
/**
 * Nonstiff DE test set problem B4, the integral surface of a torus.
 * <p>
 ...
 */          
public class B4 extends RHS {
    @Override
    public Float64Vector f(Float64 t, Float64Vector y) {
        double sqrt = Math.sqrt((y.getValue(0) * y.getValue(0) + y.getValue(1) * y.getValue(1)));
        return Float64Vector.valueOf(
            -y.getValue(1) - y.getValue(0) * y.getValue(2) / sqrt,
            y.getValue(0) - y.getValue(1) * y.getValue(2) / sqrt,
            y.getValue(0) / sqrt);
    }
